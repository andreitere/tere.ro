    ...
    <div class="mx-auto max-w-4xl space-y-6">
      <header class="mb-8 flex flex-col space-y-4 border-b-[1px] border-solid border-gray-300 pb-4">
        <div class="text-center space-y-4">
          ...
          <p class="text-muted-foreground text-sm max-w-2xl mx-auto" itemprop="description">{{ articleData?.description }}</p>
        </div>
        <div class="flex flex-wrap gap-2 items-center justify-center">
          <NuxtLink v-for="cat in articleData?.categories" :key="cat" :to="`/${cat}`"
            ...
            <span class="w-2 h-2 rounded-full" :style="`background: ${getCategoryColor(cat) || categories_colors.random}`"></span>
            {{ getDisplayName(cat) }}
          </NuxtLink>
        </div>
