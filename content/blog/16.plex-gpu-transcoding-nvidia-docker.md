...
howto:
  ...
  steps:
    - name: Install Nvidia drivers and container toolkit
      text: Install nvidia, nvidia-utils, and nvidia-container-toolkit packages using your distribution's package manager.
    - name: Configure Docker for Nvidia runtime
      text: Create or edit /etc/docker/daemon.json to add the nvidia runtime configuration, then restart the Docker service.
    - name: Verify installation
      text: Run docker info | grep runtime to confirm Nvidia runtime is available, and nvidia-smi to verify GPU is detected.
    - name: Configure Docker Compose for Plex
      text: Add the deploy.resources.reservations section with nvidia driver and video,compute,utility capabilities to your Plex container configuration.
    - name: Start Plex container
      text: Run docker compose up -d to start Plex with GPU passthrough enabled.
    - name: Verify GPU access in container
      text: Run docker exec plex ldconfig -p | grep nvidia to confirm Nvidia libraries are properly mounted inside the container.
    - name: Enable hardware transcoding in Plex
      text: Open Plex web UI, go to Settings > Transcoder, and enable Use hardware acceleration when available and Use hardware-accelerated video encoding.
    - name: Test GPU transcoding
      text: Play media that requires transcoding and run nvidia-smi to verify GPU utilization increases and Plex shows (hw) tag on active streams.
---

I've been deep into self-hosting for about 5 years now, and Plex has always been the crown jewel of my setup. My old rig was this tiny mini PC with an Intel I3 (4 cores) and 16GB of RAM, running everything through Proxmox - two VMs, one handling all my [Docker containers](/homelab/umami-portainer-traefik) including Plex. Worked like a charm for what I needed.
...
## Related Homelab Articles

If you found this useful, you might want to check out some of my other Docker and self-hosting guides:

- **[Cloudflare R2 Backup for Coolify Homelab](/homelab/cloudflare-r2-backup-coolify-homelab)** - Don't forget to backup your media library and container configurations
- **[Umami + Portainer + Traefik Setup](/homelab/umami-portainer-traefik)** - Complete container orchestration and monitoring setup  
- **[Coolify Reverse Proxy to Another Host](/homelab/coolify-reverse-proxy-to-another-host)** - Advanced networking for multi-host deployments
****