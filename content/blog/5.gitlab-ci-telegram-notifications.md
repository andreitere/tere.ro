...
howto:
  ...
  tool:
    - GitLab account
    - Telegram account
    - REST client (Insomnia, Postman, or curl)
  steps:
    - name: Create a Telegram bot
      text: Open Telegram, search for @BotFather, send /newbot command, and follow the prompts to create your bot. Save the access token provided.
    - name: Start conversation with your bot
      text: Find your new bot in Telegram and send the /start command to initiate a conversation.
    - name: Get your chat ID
      text: Make a POST request to the Telegram getUpdates API endpoint and note the message.from.id value from the response.
    - name: Test sending a message
      text: Send a POST request to the Telegram sendMessage API with chat_id and text in the JSON body to verify it works.
    - name: Add notification to GitLab CI
      text: In your .gitlab-ci.yml, add a curl command in the script section to call the Telegram sendMessage API with your pipeline status information.
    - name: Configure CI/CD variables
      text: Add ACCESS_TOKEN and CHAT_ID as protected CI/CD variables in your GitLab project settings for security.
---
